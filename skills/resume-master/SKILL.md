...
- 如果你有阅读图片的能力：请使用`scripts/pdf_to_images.py`将PDF拆分成图片，再逐页阅读图片以同时获取简历的内容和视觉效果。
- 如果你没有阅读图片的能力：告知用户并尝试将PDF内容提取为文本。
## 工作流 A：创建新简历
...
2) 如果旧简历只有 PDF：先按页拆分成图片（PNG/JPG），再使用“读取图片”工具逐页理解内容与样式，把要点记录到工作笔记里（不要将抽取内容直接作为制品交付）。
   - 拆图示例：`python scripts/pdf_to_images.py --in <old.pdf> --outdir <name>.pdf_pages --format png --dpi 200`
3) 询问样式策略：
   - 如果提供的是可编辑的 HTML 源文件：首选就地编辑以保持原有样式。
   - 如果只有 PDF/DOCX：询问用户保持原有简历风格还是选择一个模板，并从头开始编写新的 `<name>.html`。
4) 编写 `<name>.html`，并撰写 `<name>.changes.md`，内容要求：
...
你经常需要阅读PDF简历，这可能来自于用户上传的旧简历，也可能是你产出的新简历。必须先将 PDF 按页拆成图片，再逐页阅读图片以同时获取简历的视觉效果和实际内容。
